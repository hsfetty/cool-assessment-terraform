...
./SourcePoint -Host "${domain}" -Outfile "${c2_profile_location}/SourcePoint-$(date '+%Y-%m-%d')".profile -Injector NtMapViewOfSection -Stage True -Password "$password" -Keystore "${domain}.store"
